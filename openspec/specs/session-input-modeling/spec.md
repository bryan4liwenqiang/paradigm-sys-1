# session-input-modeling Specification

## Purpose
TBD - created by archiving change session-workflow-engine. Update Purpose after archive.
## Requirements
### Requirement: 混合输入模型
系统 MUST 支持在会话创建时同时接收自由文本诉求与结构化上下文字段，并将两者统一纳入会话上下文。系统 MUST 接受来自“下拉选择”与“手动输入”两种控件模式的字段值。
...
#### Scenario: 双模式字段提交
- **WHEN** 用户通过双模式控件提交字段（部分字段来自预置选项，部分来自手动输入）
- **THEN** 系统应统一接收并记录字段值
- **AND** 字段值来源不影响会话创建成功
